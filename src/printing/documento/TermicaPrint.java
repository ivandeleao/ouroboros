...
public class TermicaPrint {
    ...
    public static void gerarVenda(Venda venda, String pdfFilePath) {
        ...
        final com.itextpdf.text.Font FONT_GRANDE = new Font(FontFamily.UNDEFINED, Ouroboros.IMPRESSORA_CUPOM_TAMANHO_FONTE + 2, Font.BOLD, null);
        ...
        try {
            ...
            Paragraph nomeFantasia = new Paragraph(Ouroboros.EMPRESA_NOME_FANTASIA, FONT_NORMAL);
            nomeFantasia.setAlignment(com.itextpdf.text.Element.ALIGN_CENTER);
            pdfDocument.add(nomeFantasia);

            Paragraph razaoSocial = new Paragraph(Ouroboros.EMPRESA_RAZAO_SOCIAL, FONT_NORMAL);
            razaoSocial.setAlignment(com.itextpdf.text.Element.ALIGN_CENTER);
            pdfDocument.add(razaoSocial);

            Paragraph endereco = new Paragraph(Sistema.getEnderecoCompleto(), FONT_NORMAL);
            endereco.setAlignment(com.itextpdf.text.Element.ALIGN_CENTER);
            pdfDocument.add(endereco);
            
            Paragraph parTelefone = new Paragraph(Ouroboros.EMPRESA_TELEFONE, FONT_NORMAL);
            ...
            */
            ...
            if(!venda.getVendaTipo().equals(VendaTipo.DELIVERY) && venda.getFuncionario()!= null) {
                ...
                Paragraph parFuncionario = new Paragraph("FUNCIONÁRIO: " + funcionario.getId() + " - " + funcionario.getNome(), FONT_NORMAL);
            ...
            if(venda.getPessoa() != null) {
                ...
                Paragraph parClienteNome = new Paragraph("CLIENTE: " + pessoa.getId() + " - " + pessoa.getNome(), FONT_NORMAL);
            ...
            if(venda.getVeiculo()!= null) {
                ...
                Paragraph parVeiculo = new Paragraph("VEÍCULO: " + veiculo.getId() + " - " + veiculo.getPlaca() + " - " + veiculo.getModelo(), FONT_NORMAL);
            ...
            for(MovimentoFisico movimentoFisico : vendaItens){
                ...
                //2020-02-26 - penhão peixes pediu - concatenada a descrição na mesma linha
                Paragraph parTextoItem = new Paragraph(textoItem.trim(), FONT_NORMAL);
                parTextoItem.add(glue);
            ...
            //Total
            Paragraph total = new Paragraph("TOTAL R$", FONT_NORMAL);
            ...
            if(Ouroboros.IMPRESSORA_CUPOM_EXIBIR_MEIOS_PAGAMENTO) {
                if(!venda.getRecebimentosAgrupadosPorMeioDePagamento().isEmpty()) {
                    Paragraph tituloMeiosDePagamento = new Paragraph("MEIOS DE PAGAMENTO", FONT_NORMAL);
            ...
            if(venda.getParcelasAPrazo().size() > 0) {
                Paragraph tituloParcelas = new Paragraph("PARCELAS", FONT_NORMAL);
            ...
            if(venda.getValorTroco().compareTo(BigDecimal.ZERO) > 0) {
                ...
                Paragraph parDeliveryTroco = new Paragraph("Troco: " + Decimal.toString(venda.getValorTroco()), FONT_NORMAL);
                pdfDocument.add(parDeliveryTroco);
            }
            
            if(!venda.getComandaNome().isEmpty()) {
                Paragraph parComandaNome = new Paragraph("Nome: " + venda.getComandaNome(), FONT_NORMAL);
                pdfDocument.add(parComandaNome);
            }
            
            if(venda.getVendaTipo().equals(VendaTipo.DELIVERY)) {
                Paragraph parDeliveryEndereco = new Paragraph("Endereço: " + venda.getEnderecoEntrega(), FONT_NORMAL);
                pdfDocument.add(parDeliveryEndereco);
                        
                if(venda.getFuncionario()!= null) {
                    Funcionario entregador = venda.getFuncionario();
                    Paragraph parEntregador = new Paragraph("Entregador: " + entregador.getId() + " - " + entregador.getNome(), FONT_NORMAL);
