...
public class ContaCorrenteView extends javax.swing.JInternalFrame {
    ...
    private void carregarContas() {
        List<Conta> contas = new ContaDAO().findByTipo(ContaTipoEnum.CONTA_CORRENTE);
        ...
        if (contas.isEmpty()) {
            ...
            for (Conta c : contas) {
                System.out.println("carregar contas...");
                cboConta.addItem(c);
            }
    ...
    private void exibirTotais() {
        conta = contaDAO.findById(conta.getId()); //2020-06-04 para pegar o saldo atual
    ...
    private void entrada() {
        if (conta == null) {
            ...
            ContaSuprimentoView contaSuprimentoView = new ContaSuprimentoView(conta);
            CaixaItem caixaItem = contaSuprimentoView.getCaixaItem();
            //carregarTabela();
            if (caixaItem != null) {
    ...
    private void saida() {
        if (conta == null) {
            ...
            ContaSangriaView contaSangriaView = new ContaSangriaView(conta);
            CaixaItem caixaItem = contaSangriaView.getCaixaItem();
            //carregarTabela();
            if (caixaItem != null) {
                caixaItens.add(caixaItem);
                caixaJTableModel.addRow(caixaItem);
    ...
    private void transferir() {
        if (conta == null) {
            ...
            CaixaItem caixaItem = contaTransferirView.getCaixaItem(); //2020-06-03
            if (caixaItem != null) {
                caixaItens.add(caixaItem);
                caixaJTableModel.addRow(caixaItem);
                posicionarTabela();
    ...
    private void contas() {
        ...
        /*if (conta != null) {
            conta = contaDAO.findById(conta.getId()); //2020-06-10 para atualizar a data na tela
        }*/
