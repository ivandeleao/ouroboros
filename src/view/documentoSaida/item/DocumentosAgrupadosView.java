...
public class DocumentosAgrupadosView extends javax.swing.JDialog {
    ...
    private void tblClick() {
        if(tblDocumento.getSelectedColumn()== 9) {
            ...
            //2020-03-26 Remover Sats vinculados
            if (docRemover.getUltimoCupomValido() != null) {
                List<DocumentoReferenciado> docsReferenciados = documento.getDocumentosReferenciados().stream().filter(cupom -> cupom.getChave().equals(docRemover.getUltimoCupomValido().getChave())).collect(Collectors.toList());
                
                if (!docsReferenciados.isEmpty()) {
                    DocumentoReferenciado docRefRemover = docsReferenciados.get(0);

                    documentoReferenciadoDAO.remove(docRefRemover);

                    documento.removeDocumentoReferenciado(docRefRemover);
                }
    ...
    private void initComponents() {
        ...
        jLabel1.setText("*Ao remover documentos com Cupom Sat, estes serão automaticamente desvinculados");
