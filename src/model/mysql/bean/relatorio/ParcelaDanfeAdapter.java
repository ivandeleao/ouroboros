package model.mysql.bean.relatorio;

public class ParcelaDanfeAdapter {
    
    private String numeroA;
    private String vencimentoA;
    private String valorA;
    
    private String numeroB;
    private String vencimentoB;
    private String valorB;
    
    private String numeroC;
    private String vencimentoC;
    private String valorC;

    public String getNumeroA() {
        return numeroA;
    }

    public void setNumeroA(String numeroA) {
        this.numeroA = numeroA;
    }

    public String getVencimentoA() {
        return vencimentoA;
    }

    public void setVencimentoA(String vencimentoA) {
        this.vencimentoA = vencimentoA;
    }

    public String getValorA() {
        return valorA;
    }

    public void setValorA(String valorA) {
        this.valorA = valorA;
    }

    public String getNumeroB() {
        return numeroB;
    }

    public void setNumeroB(String numeroB) {
        this.numeroB = numeroB;
    }

    public String getVencimentoB() {
        return vencimentoB;
    }

    public void setVencimentoB(String vencimentoB) {
        this.vencimentoB = vencimentoB;
    }

    public String getValorB() {
        return valorB;
    }

    public void setValorB(String valorB) {
        this.valorB = valorB;
    }

    public String getNumeroC() {
        return numeroC;
    }

    public void setNumeroC(String numeroC) {
        this.numeroC = numeroC;
    }

    public String getVencimentoC() {
        return vencimentoC;
    }

    public void setVencimentoC(String vencimentoC) {
        this.vencimentoC = vencimentoC;
    }

    public String getValorC() {
        return valorC;
    }

    public void setValorC(String valorC) {
        this.valorC = valorC;
    }


    
}
